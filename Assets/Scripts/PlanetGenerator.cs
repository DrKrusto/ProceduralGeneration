...
public class PlanetGenerator : MonoBehaviour
{
    ...
    [SerializeField] float m_HeightCoef = 1;

    void CartesianToSpherical(Vector3 cartPos, out float rho, out float theta, out float phi)
    {
        rho = cartPos.magnitude;
        theta = 0;
        phi = 0;
        if (Mathf.Approximately(rho, 0)) return;

        phi = Mathf.Acos(cartPos.y/rho);
        theta = Mathf.Atan2(cartPos.z,cartPos.x);
    ...
    private void Awake()
    {
        ...
        m_MeshFilter.sharedMesh =    PlainSpherifiedCube(m_SpherifiedCubeNDivisions, m_SpherifiedCubeRadius, m_SpherifiedCubeRadius*.025f,
    ...
    Mesh PlainSpherifiedCube(int divisions,float radius, float heightsAmplitude,
        ComputeValueDelegate heightFunction)
        ...
        for (int face = 0; face < 6; ++face)
        {
            ...
            for (int j = 0; j < divisions + 1; ++j)
            {
                ...
                for (int i = 0; i < divisions + 1; ++i)
                {
                    ...
                    float rho, theta, phi;
                    CartesianToSpherical(vertexPos, out rho, out theta, out phi);

                    float kTheta = theta / (2 * Mathf.PI);
                    float kPhi = phi / Mathf.PI;
                    float height = radius + heightsAmplitude * heightFunction(kTheta, 1f-kPhi)* m_HeightCoef;

                    vertices[offset + i] = vertexPos.normalized * height;
                    uv[offset + i] = new Vector2(kTheta, 1f-kPhi);
